...
const StudentSupport = () => {
  const supportItems = [
    {
      icon: <Lightbulb className="w-6 h-6 text-green-800" />,
      title: "Counseling",
  ...
  return (
    <div className="bg-[#E6F4EA] py-12 px-4">
      <div className="max-w-6xl mx-auto text-center">
        <h2 className="text-3xl font-bold text-green-900 mb-4">Student Support</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {supportItems.map((item, index) => (
            <div
              key={index}
              className="bg-white shadow-md rounded-lg border border-gray-200 p-6 relative"
            >
              <div className="absolute -top-6 left-6 bg-[#C8E6C9] w-12 h-12 flex items-center justify-center rounded-full">
                {item.icon}
              </div>
              <h3 className="text-xl font-bold text-green-900 mt-8">
                {item.title}
              </h3>
              <p className="text-green-800 mt-2">{item.description}</p>
            </div>
          ))}
        </div>
      </div>

...
interface ConsultationsDisplayProps {
    data?: ConsultationData[];
    isLoading?: boolean;
}

const ConsultationsDisplay: React.FC<ConsultationsDisplayProps> = ({ data = [], isLoading = false }) => {
    ...
    const filteredData = useMemo(() => {
        return data.filter(item => {
            ...
            switch (filterType) {
                ...
                case "preferredDate":
                    return new Date(item.preferredDate).toLocaleDateString() === new Date(searchTerm).toLocaleDateString();
                default:
                    return true;
            }
        });
        ...
        <div className="flex-1 w-full min-h-screen bg-secondary-green">
            <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
                ...
                <div className="bg-white rounded-xl shadow-md p-6">
                    <h1 className="text-2xl font-bold text-gray-800 mb-2">Consultation Forms</h1>
                    <p className="text-gray-600 text-sm">
                        Showing {paginatedData.length} of {filteredData.length} consultation requests
                    </p>
                </div>
                    ...
                    <div className="flex flex-col md:flex-row gap-4">
                        <div className="flex-1">
                            <div className="relative">
                                ...
                                <input
                                    type={filterType === "preferredDate" ? "date" : "text"}
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <select
                                ...
                                onChange={(e) => setFilterType(e.target.value as "name" | "phone" | "preferredDate")}
                                value={filterType}
                            >
                                <option value="name">Filter by Name</option>
                                <option value="phone">Filter by Phone</option>
                                <option value="preferredDate">Filter by Preferred Date</option>
                            </select>
                        </div>
                    </div>
                </div>
...
export default ConsultationsDisplay;
